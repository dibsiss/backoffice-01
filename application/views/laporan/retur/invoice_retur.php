...
//print_r($detail);
        if (!empty($header)) {
            ...
            $supliyer = @$this->db->get_where('data_sumber',array('id'=>$header->id_supliyer))->row()->nama;
			(empty($supliyer))?$supliyer = 'Superadmin' : $supliyer=$supliyer;
			$keterangan = $header->keterangan;
//        $this->general->testPre($header);
        }
        if (!empty($detail)) {
            $details = $this->db->select("a.*,count(*) as jumlah_hp")->group_by('a.id_barang')->get_where('d_retur a', array('a.idh_retur' => $idTrans))->result();
            ?>
                            ...
                            <div class="box box-solid">
                                <div class="box-body">
                                    <dl>
                                        ...
                                        <dt>Jenis Retur : </dt>
                                        <dd><?php echo @$this->db->get_where('mst_retur',array('id_retur'=>$header->id_retur))->row()->status; ?>.</dd>
                                    </dl>
                                </div><!-- /.box-body -->
                            </div>
                        </div>
                            ...
                            <table id="table_client" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    foreach ($details as $de) {
                                        ...
                                        if ($isHp['is_hp'] == 1) {
                                            $getImeys = $this->db->get_where('d_retur', array('idh_retur' => $idTrans,'id_barang'=>$de->id_barang))->result();
