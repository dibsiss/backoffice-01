...
//        $this->general->testPre($header);
        ...
        if (!empty($detail)) {
            $details = $this->db->select("a.*,count(*) as jumlah_hp")->group_by('a.id_barang')->get_where('d_pengiriman a', array('a.idh_pengiriman' => $idTrans))->result();
            ?>
                            ...
                            <table id="table_client" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    foreach ($details as $de) {
                                        ...
                                        if ($isHp['is_hp'] == 1) {
                                            $getImeys = $this->db->get_where('d_pengiriman', array('idh_pengiriman' => $idTrans,'id_barang'=>$de->id_barang))->result();
