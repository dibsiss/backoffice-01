                        ...
                        <div class="col-xs-12 table-responsive">
                            <table id="table_client" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php
                                    $no = 1;
                                        $getKeterangan = $this->db->get_where('tr_koreksi',array('idtr_koreksi'=>$header->idh_transaksi))->row();                   
                                        $detailBarang = $this->master->getDetailBarang("where a.id_barang= '$header->id_barang'")->row();
                                        $isHp = $this->master->isHp($header->id_barang);
                                        if ($isHp['is_hp'] == 1) {
                                            $getImeys = $this->db->get_where('imey_koreksi', array('idtr_koreksi' => $header->idh_transaksi))->result();
                                            $imeyTampil = $this->general->listImey($getImeys, 'imey');
                                        } 
                                        ?>
                                        <tr>
                                            <td><?php echo $no++; ?>
                                            <td><?php echo $header->id_barang ?>
                                            <td><?php echo @$detailBarang->nama ?></td>
                                            
                                            <td><?php echo @$header->jumlah; ?>
                                            <td><?php echo @$getKeterangan->keterangan; ?>
